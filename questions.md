...
| A8 | **Parquet landing (CS-008).** Confirm CS-008 waits on CS-004 (JDBC/Oracle connector, Tranche 3 — spec at [[reference/cs-004-jdbc-oracle-connector-implementation]]) and the approved type mapping (§9.6.1). CS-004 build is now in progress; CS-008 is unblocked once CS-004 and Q-16 are both delivered. *(Inputs needed detailed in the **A8 note** below.)* | DAL design owner | Tranche 3/4 | DD-B3, CS-008, findings #15, #17 | ⚠️ Contract settled by CS-008; CS-004 now in build; only the §9.6.1 mapping **sign-off (Q-16)** remains open |

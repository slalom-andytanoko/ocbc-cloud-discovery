...
|---|---|---|---|---|---|
| **1** | Request to simulated completion | Request → admission (dedupe / concurrency ceiling) → simulated task execution → cancel / resume / replay → two-phase transfer-complete callback (`TRANSFER` / `DECOMPRESSION`) → terminal state | Orchestrator only | CS-001, CS-015, CS-017, **CS-018** (audit trail — every state transition written to `run_event`), CS-020 *(simulated stub — hardened in T6a)*, CS-021 *(simulated stub — real Worker impl in T3)*, CS-022, CS-026, CS-029, CS-037 *(simulated stub — hardened in T6a)*, CS-038 *(simulated stub — hardened in T6a)*, CS-040, CS-041, CS-053 *(simulated stub — real VALIDATE task in T3)*, CS-058 | ✅ Done — verified against the Orchestrator source |
